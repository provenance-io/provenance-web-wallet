...
export const addPendingRequest = createAsyncThunk(ADD_PENDING_REQUESTS, async (data: { id: string, pendingRequest: WalletconnectChromeSave }) => {
  ...
  // Save to chrome storage
  await addSavedData({ walletconnect: { ...existingData, pendingRequests, totalPendingRequests } });
  // Update badges
...
const walletConnectSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    .addCase(saveWalletconnectData.fulfilled, (state, { payload }: { payload: WalletconnectChromeSave}) => {
      ...
      if (totalPendingRequests) state.totalPendingRequests = totalPendingRequests;
    })
    ...
    .addCase(addPendingRequest.fulfilled, (state, { payload }: { payload: { totalPendingRequests: number, pendingRequests: SavedPendingRequests }}) => {
      const { totalPendingRequests, pendingRequests } = payload;
      state.pendingRequests = pendingRequests;
      state.totalPendingRequests = totalPendingRequests;
    })
    .addCase(removePendingRequest.fulfilled, (state, { payload }: { payload: { totalPendingRequests: number, pendingRequests: SavedPendingRequests }}) => {
      const { totalPendingRequests, pendingRequests } = payload;
      state.pendingRequests = pendingRequests;
      state.totalPendingRequests = totalPendingRequests;
    })
